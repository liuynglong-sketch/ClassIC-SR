...
class MSRResNetInterpTail(nn.Module):
    '''SRResNet trunk with interpolation + lightweight compensation tail.'''

    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, upscale=4, comp_channels=32):
        super(MSRResNetInterpTail, self).__init__()
        if upscale != 4:
            raise NotImplementedError('MSRResNetInterpTail currently supports x4 SR only.')
        self.upscale = upscale
        ...
        self.tail = nn.Sequential(
            ...
            nn.Conv2d(nf, comp_channels, 3, 1, 1, bias=True),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(comp_channels, out_nc, 3, 1, 1, bias=True),
        )

        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
        arch_util.initialize_weights([self.conv_first, self.tail], 0.1)
